...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${city.name} Weather Guide — Best Time to Visit, Seasons & Forecast`,
    description: `Complete ${city.name} weather guide: best time to visit (${bestMonths}), seasonal temperatures, what to pack, and hyperlocal neighborhood weather tips.`,
    alternates: { canonical: `https://cityweather.app/guides/${city.slug}` },
    openGraph: {
      title: `${city.name} Weather Guide | Best Time to Visit & Seasonal Tips`,
      description: `When to visit ${city.name}: monthly weather, seasonal breakdown, packing tips and neighborhood climate guide.`,
      url: `https://cityweather.app/guides/${city.slug}`,
    },
  }
}
...
export default async function CityGuidePage({ params }: { params: Promise<{ city: string }> }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `${city.name} Weather Guide`, item: `https://cityweather.app/guides/${city.slug}` },
    ],
  }
  ...
  const articleSchema = {
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline: `${city.name} Weather Guide`,
    description: guide?.overview ?? `Weather guide for ${city.name}`,
    url: `https://cityweather.app/guides/${city.slug}`,
  ...
  return (
    <>
      ...
      <main className="max-w-3xl mx-auto px-4 py-12">
        ...
        <section className="mb-10">
          <h2 className="text-xl font-bold text-white mb-3">Monthly Weather in {city.name}</h2>
          <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
            {['january','february','march','april','may','june','july','august','september','october','november','december'].map((month) => (
              <Link
                key={month}
                href={`/${city.slug}/weather/${month}`}
                className="bg-white/10 hover:bg-white/20 backdrop-blur rounded-xl p-2 text-center text-xs text-blue-300 hover:text-white transition capitalize"
              >
        ...
        <div className="flex gap-4 mt-8 pt-8 border-t border-white/10">
          <Link href={`/${city.slug}`} className="flex-1 text-center bg-blue-600 hover:bg-blue-500 text-white rounded-xl py-3 text-sm font-medium transition">
